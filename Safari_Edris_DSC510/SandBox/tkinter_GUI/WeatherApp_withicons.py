...
window = tk.Tk()
window.title("Get Current Weather information for any city or zip code")
HEIGHT = 500
WIDTH = 600
...
def format_weather_reporte(weather_report_json):
    try:
        ...
        final_str = 'There was a problem retrieving weather information information'
    return final_str
...
def get_a_report(zip_code_or_city):
    ...
    try:
        if is_valid_zip_code(zip_code_or_city):
            ...
            querystring = {"q": zip_code_or_city, "APPID": "f985b93ed77c52ad1dc90147bb8aa29e", 'units': 'imperial'}
        ...
        if weather_report.status_code != 200:
            if weather_report.status_code == 404:
                ...
                print('Error requesting Get ' + str(weather_report.status_code))
                exit(weather_report.status_code)
        else:
            results['text'] = format_weather_reporte(weather_report.json())
...
canvas = tk.Canvas(window, height=HEIGHT, width=WIDTH,bg='yellow')
canvas.pack()
...
get_report_frame = tk.Frame(window, bg='#42c2f4', bd=5)
...
lower_frame = tk.Frame(window, bg='#42c2f4', bd=10)
